...
public class ProductServiceImpl extends ServiceImpl<ProductMapper, Product> implements ProductService {
    ...
    public PageInfoVo productPageInfo(PmsProductQueryParam param) {
        ...
        IPage<Product> mapIPage = productMapper.selectPage(new Page<Product>(param.getPageNum(), param.getPageSize()), wrapper);
        PageInfoVo pageInfoVo = new PageInfoVo(mapIPage.getTotal(), mapIPage.getPages(), param.getPageSize(), mapIPage.getRecords(), mapIPage.getCurrent());
        return pageInfoVo;
     ...
     *
     * 解决方案看   GmallPmsApplication.java中文件
     *
     *
     * 复习：事务传播行为，
     * ====================================================================
    ...
    //rollbackFor：指定哪些异常一定回滚的。 编译时异常默认是不回滚的；
        //     *      rollbackFor：指定哪些异常一定回滚的。
    @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = Exception.class)
    void saveSkuStock(PmsProductParam productParam) {
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    void saveProductLadder(PmsProductParam productParam) {
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    void saveProductFullReduction(PmsProductParam productParam) {
    ...
    public void saveProductAttributeValue(PmsProductParam productParam) {
        ...
        valueList.forEach((item) -> {
            item.setProductAttributeId(threadLocal.get());
    ...
    public void saveBaseInfo (PmsProductParam productParam){
        ...
        log.debug("刚才商品的ID：()", product.getId());
    }
    ...
    private void saveProductToEs(Long id) {
        ...
        // b.复制sku信息，对于es要保存商品信息,还要查出这个商品的sku，给es中保存
